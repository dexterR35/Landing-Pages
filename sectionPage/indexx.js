...
function updateChart() {
    ...
    fetchData().then(datapoints => {
        const monthInfo = datapoints.MatchDayReport[0].MatchInfo.map(function (index) {
            return index.Stadium;

        });
 
            const TempInfo = datapoints.MatchDayReport[0].MatchInfo.map(function (index) {
                return index.Temp;
        ...
        myChart.config.data.MatchInfo[0].data = TempInfo;
        myChart.update();
...
//setup
    const data = {
        ...
        datasets: [{
            label: "quatar infografic",
            data: [15, 12, 5, 6, 9, 5, 9],
            backgroundColor: [
                'rgba(255,26,104,0.2',
                'rgba(54,162,235,0.2',
                'rgba(255,206,86,0.2',
