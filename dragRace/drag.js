...
let closestCar, distanceRemainingFull, distanceRemainingCar1Full, distanceRemainingCar2Full; 
//modal
let modal;
let modalText;
let modalButton;
...
function setup() {
    ...
    finishLine.addImage("finishLine", loadImage("./png/roadMobile-finish.png"));
    finishLine.scale = windowWidth / 4000;
...
function update() {
    if (gameStarted) {
        roadPosition += bgSpeed;
        startLine.position.y += bgSpeed;
        middleLine.position.y += roadPosition;
        car1.position.y -= car1Speed;
        car2.position.y -= car2Speed;
        carGood.position.y -= carGoodSpeed;
        closestCar = Math.max(distanceRemainingFull.toFixed(),distanceRemainingCar1Full.toFixed(),distanceRemainingCar2Full.toFixed());
        ...
        // Check if the car has reached 800m
         ...
         if (halfMaxDistance - closestCar >=0) {
            middleLine.y = height / 2 - ((halfMaxDistance - closestCar) * bgSpeed);
            console.log(" middleLine.y", middleLine.y)
        }
...
// text
    let textAlignM;
    var containerWidth = 220; // Set the width of the container
    var containerHeight = 70; // Set the height of the container
    var containerX = width / 2 - containerWidth / 2; // Calculate the x-coordinate of the container
    var containerY = height / 25; // Set the y-coordinate of the container
    ...
    text("Distance travelled: " + distanceTraveled.toFixed() , width / 2, containerY + 65);
    if (windowWidth <= 600) {
        textAlignM = 80;
    } else {
        textAlignM = 160
